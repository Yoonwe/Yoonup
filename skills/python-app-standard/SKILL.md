...
- **--limit 验证参数（硬性规则）**：主流程必须支持 `--limit N` 参数（argparse），用于小样本联调——流程A 查询完成后截断记录只处理前 N 条，避免全量数据（数千条）导致验证超时或重复写生产表。仅在验证时使用，正式定时执行不传（全量）。主流程模板：
  ...
  ```
- **流程A成功日志必须在 --limit 截断之后写入（实战经验）**：规范要求日志中流程A的条数在 `--limit` 模式下取截断后的实际条数。因此 `write_log("流程A - 成功 - N 条")` 必须放在 `--limit` 截断代码块**之后**，若放在截断之前会写入原始全量条数（如 100 条），与实际处理量不符
...
- **失败场景也必须写入运行记录（实战经验）**：`report_run_record` 应增加可选参数 `运行状态: str = "运行成功"`，支持传入"运行失败"。主流程 `_处理失败` 函数在发送失败通知后，必须调用 `report_run_record(ROOT_DIR, 项目名称, "运行失败")` 写入失败记录。失败记录每次新建，不参与今日成功记录的覆盖逻辑
- **共用表查询性能优化（实战经验）**：若运行记录表被多用途共用（含大量非运行记录数据），`records/search` 按运行开始时间倒序返回时可能有数千条记录，全量分页遍历会导致超时（实测单页 1.7s，10 页即 17s+）。必须在分页遍历中加入**早停优化**：遇到第一条「运行开始时间有效且早于今天零点」的记录时立即 `return ""` 停止分页（倒序排列，后面的记录只会更早）；同时加**最大页数保护**（如最多 10 页），超过则停止查询走新建逻辑
