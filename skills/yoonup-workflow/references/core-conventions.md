...
> 本文件包含 yoonup-workflow 技能的全部详细规范。
> 技能主文档只保留3条铁律+5步流程，详细内容需要时通过 MCP 工具 get_skill_spec 或查阅本文件获取。
...
## 自主优化闭环（yoonup/MCP 相关对话结束时必须执行）

### 触发条件
本次对话涉及 yoonup 工作流、MCP 服务、技能规范、部署运维等相关内容时，**不管是否实际调用了 MCP 工具**，对话结束时必须执行问题回顾与规范沉淀。

### 回顾内容（逐项自问）
1. 本次对话是否暴露了规范未覆盖的问题？
2. 是否发现工具缺陷、流程漏洞、文档不一致？
3. 是否有踩坑经验需要沉淀？
4. 是否有用户反馈的改进点？
...
- **小优化（自主执行，无需用户确认）**：
  ...
  - 错误记录新增（执行纪律章节追加）
  - 端口/路径/配置不一致修复
  - 文档间同步（references ↔ AGENTS.md ↔ skills/ 对应文件）
  - CHANGELOG 记录
- **大调整（必须向用户报告并确认后才能执行）**：
  - 架构变更（新增/删除 MCP 工具、修改工具接口）
  - 流程重写（改变 5 步流程、3 条铁律）
  - 新增技能或删除已有技能
  - 部署方式重大调整

### 执行流程
1. 修改本地 `.user_skills/yoonup-workflow/` 下对应文件
2. 同步到克隆仓库 `skills/yoonup-workflow/` 下
3. 如涉及仓库根目录文件（deploy.bat / deploy.sh / mcp_server.py / validator.py / skills.json 等），直接在克隆仓库修改
4. `git add -A && git commit -m "update: <变更简述>" && git push origin main`
5. GitHub Actions 自动部署到 Windows 服务器（`E:\yoonup`，端口 8081）
6. 验证部署成功：检查 GitHub Actions 运行状态 + MCP 服务可访问
7. 更新 CHANGELOG.md 记录本次变更

### 验证标准
- GitHub Actions 运行成功（绿色对勾）
- MCP 服务 `http://171.111.219.203:8081/mcp` 可正常响应
- 本地 `.user_skills/` 与 GitHub `skills/` 文件一致
- CHANGELOG.md 已记录本次变更

### 禁止行为
- 禁止发现问题后只留在对话上下文，不同步到规范文档
- 禁止只改本地不同步到 GitHub
- 禁止推送后不验证部署结果
- 禁止大调整不向用户确认就直接修改
- 禁止用"本次没有问题"一句话代替实际回顾
